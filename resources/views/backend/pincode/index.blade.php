...
@section('content')
    ...
    <main class="page-content">
        ...
        <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
            ...
            <div class="ms-auto">
                <form action="{{url('import_pincode_data')}}" class="d-flex justify-content-end" method="POST" enctype="multipart/form-data">
                           @csrf
            ...
            <!--<div class="ms-auto">-->
            <!--    <form action="{{url('get_pincode_data')}}" class="d-flex justify-content-end" method="POST">-->
            <!--               @csrf-->
            ...
            <div class="ms-auto">
               <a class="btn btn-primary mb-3 mb-lg-0" href="{{ url('pincode_add') }}">
                ...
                <div class="table-responsive">
                    <table id="example" class="table table-bordered" style="width:100%">
                        ...
                        @foreach($pincodes as $key=>$pincode)
                            <tr>
                                <td>{{$key+1}}</td>
                                <td>{{!empty($pincode->cities->city) ? $pincode->cities->city : ''}}</td>
                                <td>{{$pincode->pincode}}</td>
                                <td><input type="checkbox" data-pincode_id="{{$pincode->id}}" data-name="Pincode" class="pincode_status" data-toggle="toggle" data-on="Active" data-off="In Active" data-onstyle="success" data-offstyle="danger"  {{$pincode->status=='active' ? 'checked' : ''}} ></td>
                  
                                <td>
                                {{-- @foreach($owners as $owner) --}}
                                    <div class="table-actions d-flex align-items-center gap-3 fs-6">
       
                                        <a href="{{ url('pincode_edit',$pincode->id) }}" class="text-warning"
                                            data-bs-toggle="tooltip" data-bs-placement="bottom" title="Edit"><i
                                                class="bi bi-pencil-fill bg-edit" data-bs-toggle="modal"></i></a>
                                        <form action="{{url('pincode_destroy',$pincode->id)}}" method="POST">
                                            @csrf
                                            @method('DELETE')
