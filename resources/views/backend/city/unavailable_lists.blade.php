    ...
    <main class="page-content">
        ...
        <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
                    ...
                    <div class="breadcrumb-title pe-3">City List</div>
                         <form action="{{url('unavalaible_city_destroy')}}" method="POST">
                    @csrf
                    @method('POST')
                ...
                <div class="table-responsive">
                    <table id="example" class="table table-bordered" style="width:100%">
                        ...
                        @foreach($cities as $key=>$city)
                            <tr>
                                <td><input type="checkbox" class="delete_checkbox delete_checkbox{{$city->id}}" name="delete_checkbox" data-type="single" value="{{$city->id}}"></td>
                                <td>{{$key+1}}</td>
                                <td>{{$city->name}}</td>
                                <td>{{$city->pincode}}</td>
                                <td>{{$city->mobile}}</td>
