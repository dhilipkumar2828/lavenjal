...
@section('content')
    ...
    <main class="page-content">
        ...
        <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
            ...
            <div class="ms-auto">
               <a class="btn btn-primary mb-3 mb-lg-0" href="{{ url('city_add') }}">
                    ...
                    <table id="example" class="table table-bordered" style="width:100%">
                        ...
                        @foreach($cities as $key=>$city)
                            <tr>
                                <td>{{$key+1}}</td>
                                <td>{{$city->city}}</td>
                                <td><input type="checkbox" data-city_id="{{$city->id}}" data-name="Pincode" class="city_status" data-toggle="toggle" data-on="Active" data-off="In Active" data-onstyle="success" data-offstyle="danger"  {{$city->status=='active' ? 'checked' : ''}} ></td>
                  
                                <td>
                                {{-- @foreach($owners as $owner) --}}
                                    <div class="table-actions d-flex align-items-center gap-3 fs-6">
       
                                        <a href="{{ url('city_edit',$city->id) }}" class="text-warning"
                                            data-bs-toggle="tooltip" data-bs-placement="bottom" title="Edit"><i
                                                class="bi bi-pencil-fill bg-edit" data-bs-toggle="modal"></i></a>
                                        <form action="{{url('city_destroy',$city->id)}}" method="POST">
                                            @csrf
                                            @method('DELETE')
