    ...
    public function verify_otp(Request $request)
    {
        try {
            $request->validate([
                'user_id' => 'required',
                'otp' => 'required',
                'fcm_token' => 'required' // Updated field name
            ]);
            ...
            if (!empty($newToken) && strlen($newToken) > 100) {

                $existingTokens = [];

                if (!empty($user->device_key)) {
                    $decoded = json_decode($user->device_key, true);

                    if (is_array($decoded)) {
                        $existingTokens = $decoded;
                    }
                    else {
                        // fallback if corrupted old data
                        $existingTokens = [$user->device_key];
                    }
                }
                ...
                // Clean + unique
                $existingTokens = array_values(array_unique(array_filter($existingTokens)));

                $user->update([
                    'device_key' => json_encode($existingTokens)
                ]);

            }
            else {
                \Log::warning("Invalid FCM token for user: " . $user->id);
            }
    ...
    public function register(Request $request)
    {
        try {
            ...
            if (!empty($duplicate)) {
                ...
                $full_address = [];
                if (!empty($useraddress->shop_no))
                    $full_address[] = $useraddress->shop_no;
                elseif (!empty($useraddress->door_no))
                    $full_address[] = $useraddress->door_no;
                if (!empty($useraddress->address))
                    $full_address[] = $useraddress->address;
                if (!empty($useraddress->city))
                    $full_address[] = $useraddress->city;
                if (!empty($useraddress->state))
                    $full_address[] = $useraddress->state;
                if (!empty($useraddress->zip_code))
                    $full_address[] = $useraddress->zip_code;
                ...
                if ($input['user_type'] != "customer") {
                    ...
                    $meta_address = [];
                    if (!empty($useraddress->shop_no))
                        $meta_address[] = $useraddress->shop_no;
                    elseif (!empty($useraddress->door_no))
                        $meta_address[] = $useraddress->door_no;
                    if (!empty($useraddress->address))
                        $meta_address[] = $useraddress->address;
                    if (!empty($useraddress->city))
                        $meta_address[] = $useraddress->city;
                    if (!empty($useraddress->state))
                        $meta_address[] = $useraddress->state;
                    if (!empty($useraddress->zip_code))
                        $meta_address[] = $useraddress->zip_code;
