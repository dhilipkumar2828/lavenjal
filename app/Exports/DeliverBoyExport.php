...
class DeliverBoyExport implements FromCollection,WithHeadings,WithColumnWidths
{
    /**
  ...
  public function columnWidths(): array
    ...
    public function collection()
    {
        ...
        ->join('users','orders.assigned_deliveryboy','users.id')
        ...
        ->select('orders.id','orders.order_id','orders.assigned_deliveryboy','users.name','users.phone','user_addresses.door_no'
        ,'user_addresses.is_lift','user_addresses.address','user_addresses.city'
        ,'user_addresses.state','user_addresses.zip_code','order_products.quantity',
        'order_products.amount', 'orders.returnablejar_qty',DB::raw('DATE_FORMAT(orders.created_at, "%d-%b-%Y") as formatted_request')
          ,'orders.O_delivery_date','orders.delivery_time')->where('orders.assigned_deliveryboy','!=','')->where('users.user_type','delivery_agent')->where('orders.status','Delivery')->groupBy('order_products.order_id');
          ...
          foreach($orders as $order){
              ...
              if(!empty($order_products)){
                  $order->quantity=$order_products->quantity;
