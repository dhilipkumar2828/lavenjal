...
class DistributorListImport implements ToModel,WithHeadingRow
{
    /**
    * @param array $row
    *
...
//         $user=User::where('phone',(string)$row['phone'])->first();
// // if(empty($sales)){
// //   $sales=new Sales_rep();
...
//     }
   ...
   $sales=Sales_rep::where('name',$row['sales_executive'])->first();
        $user=User::where('phone',(string)$row['phone'])->where('user_type','distributor')->first();
